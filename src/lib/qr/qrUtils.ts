...
export const getPatternMask = (size: number, patternName: PatternName | "simpleMask") => {
    ...
    const setRectangular = ([startX, startY, endX, endY]: [number, number, number, number]) => {
        for (let y = startY; y < endY; y++) for (let x = startX; x < endX; x++) maskArray[y][x] = 1;
    };
     ...
     */
    const setSquare = ([startX, startY]: [number, number], size: number) =>
    ...
    if (patternName == "finder") {
        ...
        for (const coords of positions) setSquare(coords, FINDER_SIZE);
    }
...
export const combineMask = (a: Bit[][], b: Bit[][]) =>
    a.map((line, y) => line.map((bit, x) => b[y][x] | bit)) as Bit[][];
